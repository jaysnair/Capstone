...
```{r}
#Remove all rows that are not shooting death
#summary(testdata)
#testdata<-testdata[!(testdata$`Cause of Death`!="Shooting"),]
#summary(testdata)
#Cause of Death
...
```{r}
mylogit <- glm(`Cause of Death`~ Race + Age + Gender+ `Did the deceased have priors?`+ `Was the deceased armed?`, data = testdata, family = "binomial")
summary(mylogit)
```
