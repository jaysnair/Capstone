...
#Remove "Other" and "Pacific Islander" from Race.
summary(testdata$Race)
testdata<-testdata[!(testdata$Race=="Other" | testdata$Race=="Pacific Islander"),]
summary(testdata$Race)
...
#library(caret)
echo = FALSE
contro_l<- trainControl(method = "repeatedcv", repeats = 5, number = 10, verboseIter = TRUE)
...
mylogit = train(formul_a, data=train.data, method="multinom", family=binomial, trControl=contro_l, metric = "Accuracy")
...
misclasserror<-mean(testing$`Cause of Death`!= prediction)
print(paste("Accuracy", 1-misclasserror))
```


```{r}
prediction<-predict(mylogit, newdata = testing, type = "response")
```
...
#table(predict(mytree, newdata = testing), testing$`Cause of Death`)
prediction<-predict(mytree, testing)
#newtest<-cbind(testing, predict(mytree, newdata = testing))
...
```{r}
library(rpart)
mytree<-rpart(`Cause of Death`~  Gender + Age + Race + `Was the deceased armed?` + `Did the deceased have priors?`, method="class", control=rpart.control(minsplit=5, cp=0), data=train.data)

prediction<-predict(mytree, type="class")
